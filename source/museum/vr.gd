...
signal left_stick_move
signal right_stick_move
signal teleport(direction: Vector2)
...
func _process(_delta: float) -> void:
	...
	if thumbstick_vector != Vector2.ZERO:
		thumbstick_vector.x = -thumbstick_vector.x
		emit_signal("left_stick_move", thumbstick_vector)
	else:
		thumbstick_vector.x = int(Input.is_action_pressed("ui_left")) - int(Input.is_action_pressed("ui_right"))
		thumbstick_vector.y = int(Input.is_action_pressed("ui_up")) - int(Input.is_action_pressed("ui_down"))
		emit_signal("left_stick_move", thumbstick_vector)

	thumbstick_vector = $XROrigin3D/RightController.get_vector2("thumbstick")
	if thumbstick_vector != Vector2.ZERO:
		emit_signal("right_stick_move", thumbstick_vector)
...
func _on_teleport_controller_teleport(direction: Vector2) -> void:
